...
def get_2008(soup_all):
    page_df = pd.DataFrame(columns=['date',
                                   'category',
                                   'name',
                                   'change',
                                   'percentage'])
    ...
    for entry_num in range(len(soup_main_stats_category)):    
        ...
        main_category_name = entry_main_category.find('div', {'class':'stats_col_left'}).text.strip()
        
        entry_main_stats = soup_main_stats_right_cols[entry_num]
        entry_main_stats_entries = entry_main_stats.find_all('div', {'class':"stats_col_mid_details"})
...
def get_2010(soup_all):
    ...
    for entry_num in range(len(soup_main_stats_category)): 
        ...
        entry_main_stats = soup_main_stats_right_cols[entry_num]
        entry_main_stats_entries = entry_main_stats.find_all('div', {'class':"stats_col_mid data_row"})
        ...
        for single_line in entry_main_stats_entries:
            ...
            stats_line_change = single_line.find_next('div', {'class':"stats_col_right2 data_row"})
